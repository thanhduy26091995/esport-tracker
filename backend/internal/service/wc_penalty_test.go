...
func TestComputeCancelPenalty(t *testing.T) {
	tests := []struct {
		...
		{"20% of 7 = 1.4", 7, 20, true, 1.4},
		{"20% of 4 = 0.8", 4, 20, true, 0.8},
		{"50% of 1 = 0.5", 1, 50, true, 0.5},
...
func TestComputeReducePenalty(t *testing.T) {
	tests := []struct {
		...
		{
			// allowedMin = 100 - 50 = 50; 60 >= 50 → free
		...
		{
			// excess = 50-40 = 10; penalty = 10*20/100 = 2.0
		...
		{
			// excess = 50-49 = 1; penalty = 1*20/100 = 0.2
			name: "excess 1pt yields 0.2 penalty (no floor)",
			original: 100, newStake: 49, maxPercent: 50, penaltyPercent: 20,
			wantPenalty: 0.2, wantExcess: 1, wantAllowedMin: 50,
		},
		{
			// allowedMin = 100-30=70; excess = 70-60=10; penalty = 10*10/100 = 1.0
		...
		{
			// excess = 5-4 = 1; penalty = 1*20/100 = 0.2
			name: "small stake: 1-pt excess yields 0.2 penalty",
			original: 10, newStake: 4, maxPercent: 50, penaltyPercent: 20,
			wantPenalty: 0.2, wantExcess: 1, wantAllowedMin: 5,
		},
		{
			// allowedMin = 100-50=50; excess = 50-10=40; penalty = 40*50/100 = 20.0
