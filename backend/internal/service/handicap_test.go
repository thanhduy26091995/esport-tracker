...
// handicap_rate semantics: positive number = penalty subtracted from score
// eff_score = actual_score - handicap_rate
// Pro players have positive handicap_rate (e.g. 0.5): a draw means they lose effectively
...
func TestEffectiveWinner_HandicapCausesDraw(t *testing.T) {
	// Score 4-3, team1 has 1.0 handicap: eff1=3.0, eff2=3.0 → draw
	result := EffectiveWinner(4, 3, 1.0, 0)
	assert.Equal(t, 0, result, "handicap exactly bridges the gap → draw")
}

func TestEffectiveWinner_BothHandicapHigherScoreWins(t *testing.T) {
...
func TestEffectiveWinner_LargeHandicapCausesLoss(t *testing.T) {
	// Team1 won 3-1 but has 3.0 handicap: eff1=0.0, eff2=1.0 → team2 wins
	result := EffectiveWinner(3, 1, 3.0, 0)
	assert.Equal(t, 2, result, "large handicap overrides score advantage → team2 wins")
}
