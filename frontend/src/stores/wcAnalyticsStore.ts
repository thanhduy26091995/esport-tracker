...
function createAnalyticsStore(id: string) {
  return defineStore(id, () => {
    ...
    return {
      myData, communityData, compareData, wc2026Data,
      myPeriod, myDateFrom, myDateTo,
      loading, wc2026Loading, error,
      loadMyAnalytics, loadCommunityAnalytics, loadCompareAnalytics,
      loadWC2026Analytics, loadAll,
    }
  })
}

// Separate store instances so WC and AC analytics data don't share cache
const _useWcAnalyticsStoreImpl = createAnalyticsStore('wcAnalytics')
const _useAcAnalyticsStoreImpl = createAnalyticsStore('acAnalytics')

export function useWcAnalyticsStore() {
  const route = useRoute()
  return route.meta?.tournamentType === 'asean_cup' ? _useAcAnalyticsStoreImpl() : _useWcAnalyticsStoreImpl()
}
