      ...
      <div class="champion-left">
        ...
        <template v-if="myPredictions.length > 0">
          ...
          >
              ...
              <div>
                ...
                <div class="pred-meta">
                  <el-tag size="small" type="warning"
                    >{{ pred.odds_snapshot }}x</el-tag
                  >
                  <span class="pred-points">{{ pred.points }} điểm</span>
                  <span class="pred-payout"
                    >→ <strong>{{ pred.payout_if_correct }}</strong> nếu
                    đúng</span
                  >
                </div>
              </div>
            ...
            <div v-if="pred.result" class="my-pred-result">
              <el-tag
                :type="pred.result === 'correct' ? 'success' : 'danger'"
                size="small"
              >
                {{
                  pred.result === "correct" ?
                    `+${pred.points_earned} điểm`
                  : "Không đúng"
                }}
              </el-tag>
            </div>
        ...
        >
          ...
          <div class="teams-pick-grid">
            <div
              ...
              :class="{
                'team-pick-card--selected': selectedTeamId === team.id,
              }"
          ...
          <div v-if="selectedTeam" class="form-footer">
            ...
            <div class="payout-preview">
              ...
              <span class="payout-formula"
                >({{ selectedPoints }} × {{ selectedTeam.odds }}x)</span
              >
            </div>
            <el-button
              type="primary"
        ...
        >
          <div class="closed-notice">
            🔴 Cửa sổ dự đoán đã đóng — bạn chưa đặt dự đoán.
          </div>
        </el-card>

        <!-- All predictions table (below left on mobile, below form on desktop) -->
        <el-card
          v-if="allPredictions.length > 0"
          class="champion-card"
          shadow="never"
        >
          <template #header>
            <span class="card-header-title"
              >Tất cả dự đoán ({{ allPredictions.length }})</span
            >
          </template>
            ...
            <el-table :data="allPredictions" size="small" max-height="300">
              ...
              <el-table-column label="Đội" min-width="110">
                <template #default="{ row }"
                  >{{ row.flag_emoji }} {{ row.team_name }}</template
                >
              </el-table-column>
              <el-table-column
                label="Điểm"
              ...
              <el-table-column label="KQ" width="68" align="center">
                <template #default="{ row }">
                  <el-tag
                    v-if="row.result"
                    :type="row.result === 'correct' ? 'success' : 'danger'"
                    size="small"
                  >
        ...
        <el-card class="champion-card" shadow="never">
          <template #header>
            <div class="card-header-row">
              <span class="card-header-title"
                >Tỉ lệ cược ({{ teams.length }} đội)</span
              >
              <el-input
                v-model="oddsSearch"
                placeholder="Tìm..."
          ...
          <div v-else class="odds-grid">
            <div
              ...
              class="odds-chip"
              :class="{
                'odds-chip--mine': myPredictions.some(
...
const payoutPreview = computed(() =>
  selectedTeam.value ?
    Math.floor(selectedPoints.value * selectedTeam.value.odds)
  : 0,
);
const pickedTeamIds = computed(
  () => new Set(myPredictions.value.map((p) => p.team_id)),
);
...
async function handlePlace() {
  ...
  placing.value = true;
  try {
    const newPred = await wcService.placeChampionPrediction(
...
async function handleDelete(predId: string) {
  await ElMessageBox.confirm("Xóa dự đoán vô địch này?", "Xác nhận", {
    type: "warning",
  });
  try {
...
@media (max-width: 540px) {
  .champion-card :deep(.el-card__body) {
    padding: 14px 12px;
  }
