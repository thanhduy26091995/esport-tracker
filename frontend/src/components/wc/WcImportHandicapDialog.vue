<template>
  <el-dialog v-model="visible" title="Import kèo chấp từ StatsAPI" width="480px" @closed="reset">
    ...
    <div v-if="!preview" class="wc-import-intro">
      <p class="wc-import-desc">
        Lấy kèo chấp (Asian Handicap) từ TheStatsAPI và ghi đè lên dữ liệu hiện tại.
      </p>
      ...
      <div class="wc-import-compare">
        <div class="wc-import-compare-col">
          ...
          <div class="wc-import-compare-row">
            <span>Đội chấp:</span>
            <span>{{ preview.current.handicap_team ?? '—' }}</span>
          </div>
          <div class="wc-import-compare-row">
            <span>Số bàn:</span>
            <span>{{ preview.current.handicap_value ?? '—' }}</span>
          </div>
          <div class="wc-import-compare-row">
            <span>Kèo Home:</span>
            <span>{{ preview.current.odds_handicap_home ?? '—' }}</span>
          </div>
          <div class="wc-import-compare-row">
            <span>Kèo Away:</span>
            <span>{{ preview.current.odds_handicap_away ?? '—' }}</span>
          </div>
        </div>
        ...
        <div class="wc-import-compare-col wc-import-compare-col--new">
          ...
          <div class="wc-import-compare-row">
            <span>Đội chấp:</span>
            <strong>{{ preview.proposed.handicap_team ?? '—' }}</strong>
          </div>
          <div class="wc-import-compare-row">
            <span>Số bàn:</span>
            <strong>{{ preview.proposed.handicap_value ?? '—' }}</strong>
          </div>
          <div class="wc-import-compare-row">
            <span>Kèo Home:</span>
            <strong>{{ preview.proposed.odds_handicap_home ?? '—' }}</strong>
          </div>
          <div class="wc-import-compare-row">
            <span>Kèo Away:</span>
            <strong>{{ preview.proposed.odds_handicap_away ?? '—' }}</strong>
          </div>
        </div>
      </div>
...
import type { WcMatch, ImportHandicapPreview } from '@/types/wc'
...
const preview = ref<ImportHandicapPreview | null>(null)
...
async function runPreview() {
  ...
  loading.value = true
  try {
    const res = await wcService.importHandicap(match.value.id, true)
    preview.value = res as ImportHandicapPreview
  } catch (e: any) {
...
async function confirmImport() {
  ...
  saving.value = true
  try {
    await wcService.importHandicap(match.value.id, false)
    ElMessage.success('Đã import kèo chấp')
    visible.value = false
    emit('imported')
  } catch (e: any) {
